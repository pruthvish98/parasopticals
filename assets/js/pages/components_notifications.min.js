...
function showNotify(t)
{
    if (thisNotify = UIkit.notify
            ({message: t.data("message") ? t.data("message") : "", status: t.data("status") ? t.data("status") : "",
                timeout: t.attr("data-timeout") ? t.data("timeout") : 5e3,
                group: t.data("group") ? t.data("group") : null,
                pos: t.data("pos") ? t.data("pos") : "top-center",
                onClose: function () {
                    $body.find(".md-fab-wrapper").css("margin-bottom", ""),
                            t.data("callback") && executeCallback(t.data("callback")),
...
}), altair_notifications = {init: function ()
    {
        $body.on("click", "[data-message]", function () {
            var t = $(this);
            $body.find(".uk-notify-message").length ?
